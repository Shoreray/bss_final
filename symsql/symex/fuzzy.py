...
## Implemented AST nodes for division, multiplication and mod.
...
class concolic_int(int):
  ...
  def __mod__(self, o):
    res = self.__v % o
    return concolic_int(sym_mod(ast(self), ast(o)), res)

  def __rmod__(self, o):
    res = o % self.__v
    return concolic_int(sym_mod(ast(o), ast(self)), res)
  ...
  ## Implemented symbolic division, multiplication, and mod.
...
class concolic_str(str):
  ...
  ## Implemented symbolic versions of string length (override __len__)
  ...
  def replace_first(self, src, dst):
    res = self.__v.replace(src, dst, 1)
    return concolic_str(sym_replace(ast(self), ast(src), ast(dst)), res)

  def replace(self, src, dst):
    result = self
    while src in result:
        result = result.replace_first(src, dst)
    return result
...
def concolic_test(testfunc, maxiter = 1000, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    pc_prefix = const_bool(True)
    for (c, caller) in zip(cur_path_constr, cur_path_constr_callers):
      pc_to_explore = None
      if pc_prefix is not None:
        pc_to_explore = sym_and(pc_prefix, sym_not(c))
        pc_prefix = sym_and(pc_prefix, c)
      else:
        pc_to_explore = sym_not(c)
        pc_prefix = c
      if not (pc_to_explore in checked):
        (ok, model) = fork_and_check(pc_to_explore)
        if ok == z3.sat:
          new_inputs = concrete_values.copy()
          for var_name in model:
            new_inputs[var_name] = model[var_name]
          inputs.add(new_inputs, caller)
        checked.add(pc_to_explore)
    checked.add(pc_prefix)
